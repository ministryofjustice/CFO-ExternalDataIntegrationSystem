CREATE PROCEDURE [OfflocRunningPicture].[MergePNC]
AS
BEGIN
    SET NOCOUNT ON;
    BEGIN TRANSACTION;
    ...
    BEGIN TRY
        MERGE [OfflocRunningPicture].[PNC] AS target
	        USING (SELECT  DISTINCT [NOMSnumber], [Details] FROM [OfflocStagingDb].[OfflocStaging].[PNC])
