CREATE PROCEDURE [OfflocRunningPicture].[MergeStandardisationReference]
AS
BEGIN
    SET NOCOUNT ON;
    BEGIN TRANSACTION;
    ...
    BEGIN TRY
        MERGE [OfflocRunningPicture].[StandardisationReference] AS target
        USING
        (
            SELECT [RawData],
                   [CleanedData],
                   ...
                   [Source],
                   [Type]
            FROM [OfflocStagingDb].[OfflocStaging].[StandardisationReference]
        ) AS source
        ...
        (
            SELECT 
                ...
                CleanedData  
            FROM 
                [OfflocRunningPicture].[StandardisationReference] 
